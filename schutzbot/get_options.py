...
def get_files_changed():
    os.system('git remote add upstream https://github.com/jabia99/cloud-image-val.git')
    os.system('git fetch upstream')
    files_changed_cmd = ['git', 'diff', '--name-only', 'HEAD', 'upstream/skip-not-edited-files']
...
def changed_file_to_diff_list(file_changed):
    ...
    os.system(f'git diff -U10000 --output=/tmp/diff_{file_changed_underscore} HEAD upstream/skip-not-edited-files {file_changed}')
