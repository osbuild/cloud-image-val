...
class TestsGeneric:
    ...
    def test_boot_mount_presence(self, host, instance_data):
        ...
        if (
           (release_major == 8 and is_aarch64) or
           (release_major == 9) or
           (release_major >= 10 and is_azure and lvm_check) or
           is_fedora
           ):
            assert host.mountpoint("/boot").exists, "/boot mount is missing"
        else:
            assert not host.mountpoint("/boot").exists, "/boot mount is detected"
